...
/* Fade tiles out at the window edges instead of letting overflow:hidden slice
   ...
   Lifted on lock by scaling the same gradient up, keeping the winning emblem
   and glow crisp. Do NOT add a transition to mask-size: Chrome does not
   animate it, and merely declaring the transition pins the property at its
   start value, which left the fade on when locked and made it look like it
   arrived a beat into each spin. The switch is instant, hidden by the shake. */
.r6-window {
